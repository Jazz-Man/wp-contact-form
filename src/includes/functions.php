...
function wpcf7_blacklist_check($target)
{
    ...
    foreach ((array)$words as $word) {
        ...
        if (empty($word) or 256 < strlen($word)) {
            continue;
        }
...
{
    return apply_filters('wpcf7_validate_configuration', WPCF7_VALIDATE_CONFIGURATION);
}
...
function wpcf7_format_atts($atts)
{
    ...
    $prioritized_atts = ['type', 'name', 'value'];
...
function wpcf7_link($url, $anchor_text, $args = '')
{
    ...
    return sprintf('<a href="%1$s"%3$s>%2$s</a>', esc_url($url), esc_html($anchor_text), $atts ? (' ' . $atts) : '');
}
...
function wpcf7_rmdir_p($dir)
{
    ...
    if (false !== ($files = scandir($dir)) and ! array_diff($files, ['.', '..'])) {
        return rmdir($dir);
    }
...
function wpcf7_is_localhost()
{
    ...
    return in_array($server_name, ['localhost', '127.0.0.1']);
}
    ...
    if (WP_DEBUG and $trigger_error) {
        if (function_exists('__')) {
            trigger_error(sprintf(__('%1$s is <strong>deprecated</strong> since Contact Form 7 version %2$s! Use %3$s instead.',
                'contact-form-7'), $function, $version, $replacement));
        } else {
            trigger_error(sprintf('%1$s is <strong>deprecated</strong> since Contact Form 7 version %2$s! Use %3$s instead.',
...
{
    $log = sprintf(/* translators: 1: response code, 2: message, 3: body, 4: URL */ __('HTTP Response: %1$s %2$s %3$s from %4$s',
        'contact-form-7'), (int)wp_remote_retrieve_response_code($response),
        wp_remote_retrieve_response_message($response), wp_remote_retrieve_body($response), $url);

    $log = apply_filters('wpcf7_log_remote_request', $log, $url, $request, $response);
...
{
    if ( ! function_exists('inet_ntop') or ! function_exists('inet_pton')) {
        return $ip_addr;
    }
...
function wpcf7_is_file_path_in_content_dir($path)
{
    ...
    if (defined('UPLOADS') and 0 === strpos(realpath($path), realpath(ABSPATH . UPLOADS))) {
        return true;
    }
